...
const variantStyles = {
  ...
  muted: 'border border-white/10 bg-white/12 text- backdrop-blur-md hover:bg-white/16 rounded-lg',
...
export function WhiteButton({ buttonText, buttonLink, icon, className }) {
  return (
    <Button href={buttonLink} variant='light' size='md' icon={icon} className={className}>
      {buttonText}
    </Button>
  );
}
  ...
  return (
    <div className='mt-6 flex w-full flex-col gap-3 sm:mt-8 sm:w-auto sm:flex-row'>
      ...
      <Button
        ...
        size='lg'
        fullWidth
        className='text-title rounded-lg sm:min-w-[180px]'
      >
        {SecondHeroButtonText}
      </Button>
    </div>
...
export function TestimonialButton({ direction = 'left', onClick, ariaLabel, className = '' }) {
  const leftSvg = <LeftArrowIcon size={22} />;

  const rightSvg = <RightArrowIcon size={22} />;
