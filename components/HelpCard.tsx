...
const HelpCard = ({ badgeText, title, description, tabs, data }: HelpCardProps) => {
  ...
  return (
    <section className='relative z-10 mx-auto mt-20 flex min-h-[60vh] flex-col items-center justify-center'>
      ...
      <div className='relative flex w-full flex-col gap-6.5 text-center sm:w-1/2 lg:w-1/2'>
        ...
        <MiddleSectionHeads SectionHead={title} SectionDescription={description} />
      </div>
      ...
      <div className='relative mt-14 hidden w-full overflow-hidden lg:block'>
        ...
        >
          {tabs.map(tab => (
            <div key={tab} className='min-w-full shrink-0 px-5'>
              <div className='flex flex-wrap items-stretch justify-center gap-5'>
                {data[tab]?.map(card => (
                  <Card key={card.id} className='w-full max-w-[410px]'>
                    <div className='flex min-h-[400px] flex-col items-center justify-center overflow-hidden p-6'>
                      ...
                      <div className='mt-6 flex flex-col items-center gap-4 text-center'>
                        <CardTitle className='text-3xl'>{card.heading}</CardTitle>

                        <CardTitle className='text-xl text-blue-600'>{card.subheading}</CardTitle>
                      ...
                      <div className='mt-6 flex items-center justify-center gap-4'>
                        {card.icons.map(icon => (
                          <CardIcon key={icon.id}>
                            {icon.type === 'linkedin' && <LinkedInIcon />}
                          </CardIcon>
                        ))}
                      </div>
                    </div>
      ...
      <div className='bg-surface-2 mt-14 rounded-3xl border-white/10 p-4 lg:hidden'>
        <div
          key={activeTab}
          className='animate-fadeIn grid grid-cols-1 justify-items-center gap-4 sm:grid-cols-2'
        >
