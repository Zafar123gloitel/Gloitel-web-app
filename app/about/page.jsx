  ...
  return (
    <>
      ...
      <section className='relative isolate z-10 h-full w-full overflow-hidden px-4 py-16 sm:px-6 lg:px-8 lg:py-24'>
        ...
        <div className='relative z-10 mx-auto flex min-h-[60vh] flex-col items-center justify-center pt-15 text-center sm:pt-20 md:pt-20 lg:pt-20'>
          {/* Intro */}
          <IntroHead
            HomeHeroBadge='2025'
            HomeSubHeroBadge='Dig Deep About Us'
          ...
          <div className='mt-6 flex flex-col items-center gap-3 sm:mt-8 sm:flex-row sm:gap-4'>
            <Link
              href='/contact'
              className='text-md w-full rounded-md bg-white px-5 py-2.5 text-center text-black shadow-lg transition hover:bg-indigo-500 hover:text-white sm:w-auto'
            >
              Connect With Us
            </Link>
            <Link
              href='/about'
              className='text-md w-full rounded-md bg-white/10 px-5 py-2.5 text-center text-white shadow-lg backdrop-blur-md sm:w-auto'
            >
              What is Gloitel?
            </Link>
          </div>
          ...
          <div className='mt-10 w-full sm:w-3/4 lg:w-5/6'>
            <div
              className='relative cursor-pointer overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-gray-950/90 to-black/80 p-2 shadow-lg sm:p-3'
              onClick={() => setShowVideo(true)}
            >
              <div className='relative w-full pt-[56.25%]'>
                <img
                ...
                />
                <div className='absolute inset-0 flex items-center justify-center'>
                  <div className='flex h-16 w-16 items-center justify-center rounded-full bg-blue-600/90 shadow-lg sm:h-20 sm:w-20'>
                    <svg
                      xmlns='http://www.w3.org/2000/svg'
                      fill='white'
                      viewBox='0 0 24 24'
          ...
          {showVideo && (
            <>
              ...
              <div
                className='animate-fadeIn fixed inset-0 z-32 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm sm:p-6'
                onClick={() => setShowVideo(false)}
              >
                {/* Stop propagation so clicking inside doesn’t close modal */}
                <div
                  className='relative aspect-video w-full max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-black/30 shadow-2xl'
                  onClick={e => e.stopPropagation()}
                >
                  {/* YouTube Iframe */}
                  <iframe
                  ...
                  <button
                    onClick={() => setShowVideo(false)}
                    className='absolute top-0 right-0 z-32 rounded-full bg-white/20 p-2 text-white backdrop-blur-sm transition-all duration-200 hover:bg-white/40'
                    aria-label='Close video'
                  >
                    <svg
                      xmlns='http://www.w3.org/2000/svg'
      ...
      <section className='relative isolate px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-24'>
        <div className='z-30 mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 lg:grid-cols-2'>
          <div className='order-1 md:order-2'>
            <AboutAboutContent
              AboutBadge='Gloitel Vision'
              AboutHeading='Empowering Businesses'
      ...
      {/* <section className='relative isolate overflow-hidden px-4 py-12 sm:px-6 lg:px-8 lg:py-20'>
        ...
        <div className='relative z-10 mx-auto flex max-w-7xl flex-col items-center justify-center'>
          <div className='flex w-full flex-col items-center text-center sm:w-3/4 lg:w-1/2'>
            <StrategyBadge text={'Team Members'} />
            <MiddleSectionHeads
              SectionHead='Meet the Team Making'
              SectionSubHead='Things Happen Every Day'
              SectionDescription='Our team is made up of passionate professionals who bring their expertise and creativity to every project.'
          ...
          <div className='mt-12 flex w-full flex-wrap justify-center gap-6'>
            {stages.map(item => (
              <div
                key={item?.id}
                className='relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-r from-gray-950/90 to-black/80 p-2 shadow-lg sm:p-3'
              >
                <img
                  src={item?.image}
                  alt={item?.name}
                  className='h-60 w-full rounded-2xl border-2 border-gray-500/10 object-cover sm:h-72 lg:h-[400px]'
                />
                <div className='absolute inset-x-0 bottom-0 m-3 flex items-center justify-between rounded-xl border-2 border-gray-500/30 bg-gradient-to-r from-gray-900/90 to-black/80 p-3 backdrop-blur-md sm:m-5 sm:p-4'>
                  <div>
                    <h3 className='mb-1 text-sm font-semibold text-white sm:text-[16px]'>
                      {item?.name}
                    </h3>
                    <p className='text-xs text-gray-300 sm:text-sm'>{item?.role}</p>
                  </div>
                  <Link href={item?.link}>
                    <div className='flex h-10 w-10 items-center justify-center rounded-lg border-2 border-gray-400/20 bg-white/5 px-3 py-3 text-gray-400 shadow-[0_0_25px_rgba(59,130,246,0.3)]'>
                      {item?.icon}
                    </div>
                  </Link>
                </div>
      ...
      <HorizontalDivider />
      {/* Culture Section */}
      <div className='relative flex flex-col items-center overflow-hidden px-4 py-12 sm:px-6 lg:px-8 lg:py-20'>
        <BgSquare3 />
        <div className='flex w-full flex-col items-center text-center sm:w-3/4 lg:w-1/2'>
          <StrategyBadge text='Our Culture' />
          <MiddleSectionHeads
